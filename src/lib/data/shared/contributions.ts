 ...
 * State, stars and line counts are refreshed from GitHub on every build (scripts/sync-contributions.mjs),
 * so the values below are only the fallback. Text per contribution lives in locales/<lang>/contributions.ts.
 */
export type DiffRow = { k: 'hunk'; t: string } | { k: 'gap' } | { k: ' ' | '+' | '-'; t: string; o?: number; n?: number };
...
const written: Contribution[] = [
  {
    ...
    deletions: 56,
    files: 9,
    tags: ['ML-DSA-65', 'FIPS 204', 'Crypto-agility', 'JWT'],
    diff: {
  ...
  },
  {
    ...
    deletions: 2,
    files: 2,
    tags: ['PKI', 'cert-manager', 'Kubernetes', 'RSA-4096'],
    diff: {
  ...
  },
  {
    ...
    deletions: 12,
    files: 5,
    tags: ['ML-KEM', 'X25519MLKEM768', 'AES-256-GCM', 'TLS 1.3'],
    diff: {
...
type Live = Pick<Contribution, 'state' | 'title' | 'additions' | 'deletions' | 'files' | 'stars'>;
// Written by the build script and git-ignored; the glob is empty when the file does not exist.
const liveFile = import.meta.glob<Record<string, Live>>('./contributions.live.json', { eager: true, import: 'default' });
const live = Object.values(liveFile)[0] ?? {};

export const contributions: Contribution[] = written.map((c) => ({ ...c, ...live[c.id] }));
